...
(* these functions are just for cleaning up error printing. They each will print
   a different message depending on what error is caught *)
let e_s () =
...
(* is_op and is_num will tell us if we are an operator or a value to put on its
   respective stack *)
let is_op = fun token ->
  match token with
    Plus_tok -> true
